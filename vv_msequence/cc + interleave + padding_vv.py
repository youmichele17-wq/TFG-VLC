...
def process_chain(bits, rate):
    b_int = interleave_and_puncture(bits, INTER_ROWS)
...
def build_packet(seq, seed):
    ...
    payload_bits = bytes_to_bits(bits_to_bytes(seq))
    seq_bytes = bits_to_bytes(payload_bits)
...
def main():
    ...
    print(f"V&V - CC + Interleave + Padding - Payload = {PAYLOAD_RATE}")
    ...
    while True:
        try:
            ...
            for t in range(N):
                ...
                msg = mseq()
                tx_bytes, payload_bits_tx, payload_coded_tx, header_coded_tx, header_bits_tx, psdu_tx, psdu_rx, start_payload = build_packet(msg, seed)
                ...
                HEADER_RATE = "1/4"
                header_info_bits_len = len(header_bits_tx)
                hdr_coded_len = coded_len(header_info_bits_len, HEADER_RATE)

                if len(frame) < ptr + hdr_coded_len:
                    fail_hdr += 1
                    continue

                hdr_coded = frame[ptr: ptr + hdr_coded_len]

                hdr_bits_int = viterbi(depuncture(hdr_coded, HEADER_RATE))

                hdr_bits = deinterleave_with_insertion(hdr_bits_int, header_info_bits_len, rows=INTER_ROWS)

                hdr_bytes = bits_to_bytes(hdr_bits)[:6]
                if len(hdr_bytes) < 6:
                    fail_hdr += 1
                    continue
                ...
                ptr_pl = ptr + hdr_coded_len

                payload_info_bits_len = len(payload_bits_tx)
                payload_coded_len = coded_len(payload_info_bits_len, rate)

                if len(frame) < ptr_pl + payload_coded_len:
                    fail_pl += 1
                    continue

                payload_coded = frame[ptr_pl: ptr_pl + payload_coded_len]

                payload_bits_int = viterbi(depuncture(payload_coded, rate))
                payload_bits_rx = deinterleave_with_insertion(payload_bits_int, payload_info_bits_len, rows=INTER_ROWS)

                msg_bytes = bits_to_bytes(payload_bits_rx)[:length]
                ...
                err2 = sum((a != b) for a, b in zip(hdr_bits_post, hdr_bits_ref))
