...
SHR = FLP + TDP + TDP_INV + TDP + TDP_INV
...
def bits16_to_u16_lsb(bits16: List[int]) -> int:
    v = 0
...
def nibbles_to_bytes_low_first(nibs: List[int]) -> bytes:
    out = bytearray()
...
def rs_encode_shortened_syms(data_syms: List[int]) -> List[int]:
    out = []
    i = 0

    while i < len(data_syms):
        blk = data_syms[i:i + HEADER_RS_K]

        if len(blk) == HEADER_RS_K:
            out.extend(list(RS_7.encode(bytes(blk))))
            i += HEADER_RS_K
...
def rs_decode_shortened_syms(
    ...
    nsym = RS_N - HEADER_RS_K

    full = orig_len_syms // HEADER_RS_K
    rem = orig_len_syms % HEADER_RS_K
...
def build_phr_bits(
    ...
    phr.extend(int_to_bits_lsb(channel, 3))
    phr.extend(int_to_bits_lsb(mcs_id, 6))
    phr.extend(int_to_bits_lsb(payload_len_bytes, 16))
...
def encode_header_mcs0(
    ...
    hdr_bits = list(phr_bits_32) + int_to_bits_lsb(hcs, 16)

    hdr_bytes = bits_to_bytes_lsb(hdr_bits)[:HEADER_BYTES]

    hdr_syms = bytes_to_nibbles_low_first(hdr_bytes)

    rs_syms = rs_encode_shortened_syms(hdr_syms)
    ...
    return header_coded_bits, srs, len(hdr_syms)
...
def main():
    ...
    header_coded_tx, srs_header, header_len_syms = encode_header_mcs0(
        phr_bits
    )
    ...
    rs_syms_in = decode_header_to_rs_symbols(
        header_coded_rx,
        srs_header
    )

    rs_syms_in = corrupt_rs_symbols_per_cw(
        rs_syms_in,
        RS_SYM_ERR_HEADER,
        rng
    )
    ...
    try:
        header_syms_rx = rs_decode_shortened_syms(
            rs_syms_in,
            orig_len_syms=header_len_syms,
            strict=True
        )

        header_rx = nibbles_to_bytes_low_first(
            header_syms_rx
        )[:HEADER_BYTES]
    ...
    if ok_header_rs and len(header_rx) >= HEADER_BYTES:
        phr_rx_bits = bytes_to_bits_lsb(
            header_rx[:4]
        )[:32]

        got_hcs = bits16_to_u16_lsb(
            bytes_to_bits_lsb(header_rx[4:6])[:16]
        )

        calc_hcs = crc16_hcs_lsb(phr_rx_bits)
    ...
    if phr_ok:
        payload_rx = PAYLOAD_BYTES
        ok_payload = payload_rx == PAYLOAD_BYTES

    resultado = ok_header_rs and phr_ok and ok_payload

    print(f"PAYLOAD_RATE={PAYLOAD_RATE}")
    print(f"BIT_ERR_HEADER={BIT_ERR_HEADER}")
    print(f"RS_SYM_ERR_HEADER={RS_SYM_ERR_HEADER}")
    print()
